...
public class Ball : MonoBehaviour 
{
	...
	void Update ()
	{
		...
		if(transform.position.x > 8 && !goingLeft){					//Is the ball at the right border and is not going left (heading towards the right border)
		...
		if(transform.position.x < -8 && goingLeft){					//Is the ball at the left border and is going left (heading towards the left border)
		...
		if(transform.position.y < -5) {								//Has the ball gone down past the paddle
			manager.balls.Remove(gameObject);
		    manager.DestroyBall();
			Destroy(gameObject);
